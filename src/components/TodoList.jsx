...
const TodoList = () => {
    ...
    function addTodoLocalStorage(newTodo){
        ...
        todos= [...todos,{ id: gentId(todos), title: newTodo.text,completed:false}]
    ...
    const completeTodo =(id) =>{
        setResultTodos(resultTodos.map(todo=>{
            if(todo.id === id){
                return {
                    ...todo,
  ...
  return (
    <div className='todo-container'>
        ...
        {resultTodos.length!==0 ? <div className='todos'>
        {resultTodos.map(todo=>(
            <Todo key={todo.id} todo={todo} completeTodo ={completeTodo} removeTodo = {removeTodo}/>
            ))}
            <Footer count = {count} removeAllTodo={removeAllTodo} todos = {resultTodos} />
             </div> : <div className='emptys'>
             <EmptyTodo />
             <Footer count = {count} removeAllTodo={removeAllTodo} todos = {resultTodos} />
                </div>}
    </div>
